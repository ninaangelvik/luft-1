...
var pollutionColors = map[string]string{
	"low": "#6ee86e",
	"moderate": "#ff9900",
	"high": "#ff0000",
	"extreme": "#990099", 
...
func StudentAqisHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println("in student aqi handler")
	values := r.URL.Query()
	...
	for _, measurement := range data {
		color := getPollutionColor(measurement.PmTen, measurement.PmTwoFive)

		geom := geojson.NewPointGeometry([]float64{measurement.Longitude, measurement.Latitude})
		f := geojson.NewFeature(geom)
		f.SetProperty("student", "true")
		f.SetProperty("date", measurement.Date)
		f.SetProperty("pm10", measurement.PmTen)
		f.SetProperty("pm2.5", measurement.PmTwoFive)
...
func getPollutionColor(pmTen string, pmTwoFive string) string {
